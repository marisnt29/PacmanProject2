...
FOOD_SIZE = 0.1
# Door
DOOR_COLOR = formatColor(139/255.0, 69/255.0, 19/255.0)
DOOR_SIZE = 0.5
# Key
KEY_COLOR = formatColor(255/255.0, 215/255.0, 0/255.0)
KEY_SIZE = 0.5
# Laser
...
class PacmanGraphics:
    ...
    def drawStaticObjects(self, state):
        ...
        self.doors = self.drawDoors(layout.doors)
        self.keys = self.drawKeys(layout.keys)
        refresh()
    ...
    def update(self, newState):
        ...
        if newState._keyCollected != None:
            self.removeKey(newState._keyCollected,self.keys)
        if newState._doorOpened != None :
            self.removeKey(newState._doorOpened,self.doors)
    ...
    def drawDoors(self, doorsMatrix):
        doorsImages = []
        color = DOOR_COLOR
        for xNum, x in enumerate(doorsMatrix):
            imageRow = []
            doorsImages.append(imageRow)
            for yNum, cell in enumerate(x):
                if cell:  # There's door there
                    screen = self.to_screen((xNum, yNum))
                    dot = circle(screen,
                                 DOOR_SIZE * self.gridSize,
                                 outlineColor=color, fillColor=color,
                                 width=1)
                    imageRow.append(dot)
                else:
    ...
    def drawKeys(self, keysMatrix):
        keysImages = []
        color = KEY_COLOR
        for xNum, x in enumerate(keysMatrix):
            imageRow = []
            keysImages.append(imageRow)
            for yNum, cell in enumerate(x):
                if cell:  # There's door there
                    screen = self.to_screen((xNum, yNum))
                    dot = circle(screen,
                                 KEY_SIZE * self.gridSize,
                                 outlineColor=color, fillColor=color,
                                 width=1)
                    imageRow.append(dot)
                else:
    ...
    def removeKey(self, cell, keysImages):
        x, y = cell
        remove_from_screen(keysImages[x][y])

    def removeDoor(self, cell, doorsImages):
        x, y = cell
        remove_from_screen(doorsImages[x][y])

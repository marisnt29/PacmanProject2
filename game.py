...
class Actions:
    ...
    def getPossibleActions(config, walls,doors,flag):
        possible = []
        x, y = config.pos
        ...
        for dir, vec in Actions._directionsAsList:
            ...
            if flag:
                if not walls[next_x][next_y]:
                    possible.append(dir)
            if not flag:
                if not walls[next_x][next_y] and not doors[next_x][next_y]:
                    possible.append(dir)
    ...
    def getLegalNeighbors(position, walls,doors,flag):
        x, y = position
        x_int, y_int = int(x + 0.5), int(y + 0.5)
        neighbors = []
        for dir, vec in Actions._directionsAsList:
...
class GameStateData:
    ...
    def __init__( self, prevState = None ):
        ...
        """
        if prevState != None:
            self.keys = prevState.keys.shallowCopy()
            self.doors = prevState.doors.shallowCopy()
            ...
            self.availableKeys = prevState.availableKeys


        self._foodEaten = None
        self._keyCollected = None
        self._doorOpened = None
        ...
        self._win = False
        self.scoreChange = 0
        self.availableKeysChange = 0


    def deepCopy( self ):
        state = GameStateData(self)
        state.food = self.food.deepCopy()
        state.keys = self.keys.deepCopy()
        state.doors = self.doors.deepCopy()
        state.layout = self.layout.deepCopy()
        state.flag = self.flag


        state._agentMoved = self._agentMoved
        state._foodEaten = self._foodEaten
        state._keyCollected = self._keyCollected
        state._doorOpened = self._doorOpened
    ...
    def initialize( self, layout, numGhostAgents ):
        ...
        self.keys = layout.keys.copy()
        self.doors = layout.doors.copy()

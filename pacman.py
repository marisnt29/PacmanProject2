...
#        GameState.explored.add(self)
    ...
    def generateSuccessor(self, agentIndex, action):
        ...
        state.data.availableKeys += state.data.availableKeysChange
        GameState.explored.add(self)
    ...
    def getAvailableKeys(self):
        return self.data.availableKeys

    def getKeys(self):
        return self.data.keys

    def getDoors(self):
        return self.data.doors
...
class PacmanRules:
    ...
    def getLegalActions(state):
        ...
        """
        return Actions.getPossibleActions(state.getPacmanState().configuration, state.data.layout.walls,state.data.doors,state.data.flag)
    ...
    def consume(position, state):
        ...
        if state.data.availableKeys < 1:
            GameState.closeTheDoors(state)

        if state.data.keys[x][y]:
            state.data.keys = state.data.keys.copy()
            state.data.keys[x][y] = False
            state.data._keyCollected = position
            state.data.availableKeysChange += 1

            GameState.openTheDoors(state)


        #Open Door
        if state.data.doors[x][y] and state.data.availableKeys >= 1:
            state.data.doors = state.data.doors.copy()
            state.data.doors[x][y] = False
            state.data._doorOpened  = position
            state.data.availableKeysChange -= 1
            if state.data.availableKeys < 1:
                GameState.closeTheDoors(state)
...
class GhostRules:
    ...
    def getLegalActions(state, ghostIndex):
        ...
        possibleActions = Actions.getPossibleActions(
            conf, state.data.layout.walls,state.data.doors,state.data.flag)
